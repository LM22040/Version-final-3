...
class WhatsAppAccessibilityService : AccessibilityService() {
    ...
    private fun toggleTranslation() {
        ...
        if (isTranslating) {
            translateScreen()
            bubbleButton?.setImageResource(R.drawable.ic_translate_active) // Cambia el ícono cuando está activado
            bubbleButton?.startAnimation(AnimationUtils.loadAnimation(this, R.anim.scale_up)) // Aplica la animación
        } else {
            restoreOriginalText()
            bubbleButton?.setImageResource(R.drawable.ic_translate) // Cambia el ícono cuando está desactivado
            bubbleButton?.startAnimation(AnimationUtils.loadAnimation(this, R.anim.scale_down)) // Aplica la animación inversa
        }
    }
    ...
    private fun translateScreen() {
        ...
        originalText = extractTextFromNode(rootNode)
        if (originalText.isNotEmpty()) {
            CoroutineScope(Dispatchers.IO).launch {
                val translatedText = translateText(originalText)
                CoroutineScope(Dispatchers.Main).launch {
                    showTranslationDialog(originalText, translatedText)
                }
            }
        }
    }
